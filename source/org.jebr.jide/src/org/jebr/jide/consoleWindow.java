...
public class consoleWindow extends JFrame implements ItemListener, WindowListener, ActionListener, AttributeSet.FontAttribute{
	 ...
	 */
        public void set()
        {
             toolCompile.setBorder(BorderFactory.createEmptyBorder());
        }
	    /*Hii*/
	...
	public consoleWindow(String name) {
		...
		setContentPane(contentPane);
                ...
                dark2.doClick(); // set the default theme :)
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if (e.getSource()==Compile)
       {
           ...
           try {
               ...
               out = new PrintStream(nameOfFile+"/stdlib.jlb");
               System.setOut(out);
               ...
               while(z.hasNextLine())
               {
                   ...
                   if (count == 1 )
                   {
                       doc.insertString(doc.getLength(), "JINT Parsing Log:" + "\n", keyWord );
                       doc.insertString(doc.getLength(),  "\n", keyWord );
                       doc.insertString(doc.getLength(), z.nextLine() + "\n", null );
               ...
               if (count == 0)
               {
                   doc.insertString(doc.getLength(), "JINT Parsing Log:" + "\n", keyWord );
                   doc.insertString(doc.getLength(),  "\n", keyWord );
                   doc.insertString(doc.getLength(), "Build Successful \n", null );
               }
               doc.insertString(doc.getLength(), "Finished Parsing at "+dateFormat.format(date)+" \n", null );
               count = 0 ;
       ...
       if(e.getSource()==Open)
       {
           ...
           try{
            ...
            FileNameExtensionFilter jlbfilter = new FileNameExtensionFilter("jlb files (*.jlb)", "jlb");
			open.setFileFilter(jlbfilter);
			open.addChoosableFileFilter(jlbfilter);
            open.setDialogTitle("Open a jlb file into prespective");
 // set selected filter
            open.setFileFilter(jlbfilter);
           ...
           {
               JOptionPane.showMessageDialog(null, "Jim, The file you've chosen isn't a jlb file  ", "Error",JOptionPane.OK_OPTION);
               path=null;
           }
            ...
            if (path==null)
            {
            	JOptionPane.showMessageDialog(null, "Error 1516: An unexpected error occured", "ERROR!", JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    public void itemStateChanged(ItemEvent ie) {
       ...
       if (dark.isSelected()) //Night Sparkle
       {
